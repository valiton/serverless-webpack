...
const DEP_PROPERTIES = ['dependencies', 'devDependencies', 'peerDependencies', 'bundledDependencies', 'optionalDependencies'];

function getPackageContent(packagePath) {
  return require(path.join(process.cwd(), packagePath));
}

function getProdModules(externalModules, packagePath) {

  const packageJson = getPackageContent(packagePath);
...
module.exports = {
  packExternalModules(stats) {
    ...
    return BbPromise.resolve().then(() => {
      ...
      let pkg = getPackageContent(packagePath);
      DEP_PROPERTIES.forEach((item) => delete pkg[item]);
      fs.writeFileSync(tmpPackageJson, JSON.stringify(pkg, null, 4));
